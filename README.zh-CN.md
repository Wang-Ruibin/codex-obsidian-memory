...
- 分支名按完整名称和大小写精确匹配。对于 `Release` 和 `release`，在不区分文件名大小写的系统上，请使用 `release-upper.md` 和 `release-lower.md` 等不同笔记文件名。
...
| 分支名只在字母大小写上不同 | 使用不同的笔记文件名，在各笔记内保留精确分支名；让 Codex 检查对应关系。 |
...
| Codex 被再次要求补全回写审查 | 每个修改的笔记都需要单独一条列表项，写清相对知识库的路径和具体改动；只有文件名或“已更新”不够，让 Codex 补全提示中列出的缺失项。 |
